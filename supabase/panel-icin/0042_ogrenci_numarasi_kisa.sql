-- SEKİZ — 0042: Öğrenci numarası
-- Supabase panelinde SQL Editor'a yapıştırıp Run deyin.
-- Beklenen sonuç: "Success. No rows returned."
-- (Panel sonuç tablosunda NULL gösterebilir — bu da başarı demek.)
-- Açıklamalı tam sürüm: supabase/migrations/0042_ogrenci_numarasi.sql
--
-- ÖNCE YEDEK ALIN. (Öğretmen ekranı → Yedek)
-- ÖNCE SİTE YAYINA ALINMIŞ OLMALI.
--
-- BU DOSYA ÇALIŞTIRILANA KADAR ÖĞRENCİ LİSTELERİ VE TOPLU EKLEME HATA
-- VERİR: bu turda uçların çıktısı değişiyor. Siteyi yayına aldıktan sonra
-- bunu hemen çalıştırın.
--
-- NE YAPIYOR: Öğrencilere okul numarası alanı ekliyor. e-Okul PDF'inden
-- aktarırken numara artık adın içinde kalmıyor, kendi alanına gidiyor ve
-- öğrenci listelerinde adın yanında görünüyor.
--
-- NUMARASI OLMAYAN ÖĞRENCİ SORUN DEĞİL: özel ders öğrencisinin okul
-- numarası yoktur, alan boş kalır ve ekranda hiçbir şey yazmaz.
--
-- AYNI NUMARA ENGELLENMİYOR: sizin kararınızdı — önizlemede uyarı çıkar,
-- kararı siz verirsiniz. Yanlış okunan tek bir numara yüzünden bütün
-- sınıfın reddedilmesini istemiyoruz.
--
-- Veri silinmiyor, mevcut öğrenciler etkilenmiyor (numaraları boş kalır).
