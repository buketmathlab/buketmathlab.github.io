-- SEKİZ — 0053: eksik konu tekten listeye (en fazla 3 başlık)
-- Supabase panelinde SQL Editor'a yapıştırıp Run deyin.
-- Açıklamalı tam sürüm: supabase/migrations/0053_eksik_konular.sql
--
-- ÖNCE YEDEK ALIN. (Öğretmen ekranı → Yedek)
-- BU SQL ÖNCE ÇALIŞMALI, SİTE SONRA YAYINA ALINMALI.
-- 0051 ve 0052 ÇALIŞMIŞ OLMALI.
--
-- NE YAPIYOR: uç şimdiye kadar öğrenci başına TEK bir konu adı
-- döndürüyordu. Yazdırılan kâğıtta "eksik olduğu konu BAŞLIKLARI"
-- istediğiniz için (çoğul), alan bir listeye dönüyor: en eksikten
-- başlayarak EN FAZLA 3 konu.
--
-- Ekranda değişiklik yok — Öğrenciler sekmesi listenin ilkini
-- gösteriyor, yani bugünkü davranışın aynısı. Üç başlığın tamamı
-- yazdırılan kâğıtta.
--
-- ÖLÇÜT DEĞİŞMEDİ: yalnız toplam 5+ soru çözülmüş ve yanlışı olan
-- konular aday; hiç yanlışı olmayan öğrenciye konu yazılmıyor.
--
-- Veri SİLİNMİYOR, hiçbir tablo değişmiyor; tek bir fonksiyon yenileniyor.
--
-- Beklenen sonuç: en altta tek satırlık bir tablo (_migration_kaydet → 0053).
-- =============================================================================
