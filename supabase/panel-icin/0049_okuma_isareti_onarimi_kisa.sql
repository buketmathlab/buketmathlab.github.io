-- SEKİZ — 0049: "Bu bölüm yüklenemedi" onarımı
-- Supabase panelinde SQL Editor'a yapıştırıp Run deyin.
-- Açıklamalı tam sürüm: supabase/migrations/0049_sinif_velileri_kanal.sql
--
-- ÖNCE YEDEK ALIN. (Öğretmen ekranı → Yedek)
--
-- NE OLDU: Veliler → 9A dediğinizde ekran "Bu bölüm yüklenemedi" dedi.
-- Sebep, okuma işaretlerini tutan tabloda aynı kişi için birden çok satır
-- olabilmesi; ekranı çizen sorgu tek satır bekliyordu.
--
-- İkinci satır, Mesajlar sekmesinde aynı öğrencinin hem veli hem öğrenci
-- kanalını açtığınızda oluşuyor. Hata eskiden beri vardı ama iki kanal
-- ayrı yerlerde olduğu için kimse ikisine birden basmıyordu.
--
-- AYNI HATANIN İKİNCİ BİR YOLU DA BULUNDU: bir sınıf başka bir öğretmene
-- devredilirse öğrencinin KENDİ panosu da aynı şekilde patlıyordu. Henüz
-- yaşanmadı; bu SQL onu da kapatıyor.
--
-- BİR SAYI DA DÜZELİYOR: sınıf veli listesindeki "N mesaj", öğrenci
-- kanalındaki ve varsa meslektaşınızın yazdığı mesajları da sayıyordu.
-- Artık tıklayınca açılan yazışmayla aynı sayıyı gösteriyor.
--
-- VERİ SİLİNMİYOR. Yalnız aynı kişi için birikmiş çift okuma işaretleri
-- birleştiriliyor; en son bakılan zaman korunuyor.
--
-- Beklenen sonuç: en altta tek satırlık bir tablo (_migration_kaydet → 0049).
-- =============================================================================
