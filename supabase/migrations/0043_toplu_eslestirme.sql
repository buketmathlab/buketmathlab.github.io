-- =============================================================================
-- 0043 — TOPLU EKLEMEDE MEVCUT ÖĞRENCİYİ EŞLEŞTİR
--
-- NE OLDU
-- Öğretmen sınıfları önce NUMARASIZ eklemişti. 0042 yayına girince aynı
-- e-Okul listelerini numaralarla yeniden yükledi ve **her sınıf iki katına
-- çıktı**: aynı çocuk hem numarasız (eski) hem numaralı (yeni) kayıtla.
--
-- BUNU ÜRÜN YAPTI, ÖĞRETMEN DEĞİL. Önizleme "Sınıfta kayıtlı" diye
-- uyarıyordu ama uyarı kaydetmeyi engellemiyordu ve *"bu zaten var,
-- numarasını yaz"* diyen bir yol hiç yoktu. Eksik olan tasarımdı.
--
-- BU DOSYA O YOLU AÇIYOR. `ogrenciler_toplu_ekle` artık, istendiğinde,
-- sınıfta zaten duran öğrenciyi tanıyıp **numarasını yazıyor; yeni kayıt
-- açmıyor**.
--
-- (Oluşmuş kopyaların temizliği ayrı iş: `panel-icin/kopya-ogrenci-*.sql`.)
-- =============================================================================
