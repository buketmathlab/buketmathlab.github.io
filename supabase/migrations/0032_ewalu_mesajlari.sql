...
--
-- Bugüne kadar Ewalu'nun beş cümlesi KODA GÖMÜLÜYDÜ (`lib/ewalu-puan.ts`);
-- bir kelimesini değiştirmek bir geliştirme turu gerektiriyordu. Bu tur
-- öğretmene o cümleleri kendi yazma ve istediği an varsayılana döndürme
-- imkânı veriyor.
--
-- TABLO BOŞ BAŞLIYOR VE BU TASARIMIN ÇEKİRDEĞİ.
--
-- Varsayılan beş cümle KODDA kalıyor; bu tablo yalnız "öğretmen bu bandı
-- DEĞİŞTİRDİ" bilgisini taşıyor. Üç sonucu var:
--
--   1. Migration hiçbir metni sahiplenmiyor. Cümlelerin tek doğruluk
--      kaynağı `lib/ewalu-puan.ts` olmayı sürdürüyor; iki yerde iki farklı
--      "varsayılan" oluşup zamanla ayrışmıyor.
--   2. 0032 çalıştırılmasa bile ekran bugünkü gibi çalışıyor — uç yoksa
--      istemci varsayılana düşüyor (Part VIII: yedek davranış).
--   3. "Varsayılana dön" satırı SİLMEK demek. Ayrı bir bayrak ya da
--      varsayılan metni tabloya geri yazmak gerekmiyor.
--
-- NEDEN AYRI TABLO — `ayarlar`'a KOYULMADI, ölçülerek karar verildi:
--
--   `disa_aktar` (0004:911) sekiz tabloyu yediliyor ve `ayarlar` ARALARINDA
--   YOK. Cümleleri oraya koysaydım öğretmenin kendi yazdığı metinler yedeğe
--   hiç girmez, bir geri yüklemede SESSİZCE kaybolurdu. `ayarlar`'ı
--   `disa_aktar`'a eklemek de yanlış olurdu: içinde `ogretmen_pin_hash` var
--   ve `docs/yedekleme.md` PIN'in yedekte BULUNMAMASINI açık bir güvence
--   olarak yazıyor — yedek dosyası kişisel buluta ve e-postaya gidiyor.
--
--   Ayrı tablo ikisini birden çözüyor: içinde sır yok, yedeğe giriyor.
--   Bu migration `disa_aktar`'ı da güncelliyor (bölüm 4).
--
-- `ogrenci_odevleri`'NE DOKUNULMUYOR — bilinçli. Cümleyi o yanıta eklemek
-- 300 satırlık bir gövdeyi birebir kopyalamayı gerektirirdi; 0016'da
-- ezberden gövde yazmak iki hataya yol açmıştı ve `ogrenci_odevleri`
-- öğrencinin en kritik ucu. Ayrı ve küçük bir okuma ucu, 0031'de
-- `odev_guncelle`'ye dokunmama kararının aynısı.
-- =============================================================================

-- -----------------------------------------------------------------------------
-- 1. Tablo
--
-- `bant` birincil anahtar: her band için EN FAZLA bir özel cümle. Değerler
-- `lib/ewalu-puan.ts`'teki `enAz` değerleriyle birebir aynı; kısıt onları
-- kilitliyor ki iki taraf sessizce ayrışmasın.
--
-- 400 KARAKTER SINIRI: bugünkü en uzun cümle 158 karakter, yani sınır rahat.
-- Sınırsız bırakmak, sonuç kartını taşıracak bir metnin sunucuya kabul
-- edilmesi demek olurdu; sınırı ekran değil sunucu koyuyor (Part XXI).
-- -----------------------------------------------------------------------------
