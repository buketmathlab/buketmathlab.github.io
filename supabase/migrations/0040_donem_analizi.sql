-- =============================================================================
-- 0040 — HAFTALIK, AYLIK VE DÖNEMLİK SINIF ANALİZİ
--
-- NE İSTENDİ
-- Öğretmen: "Her öğretmenin verdiği ödevinden haftalık analiz oluşsun.
-- Mesela 9A'ya verdiği ödevlerden o haftanın ödevlerinin ortalaması,
-- haftalık olarak iyi olan konular, çalışılması gereken konular gibi
-- haftalık ve aylık analizler otomatik gelsin. Dönem sonunda da o dönemin
-- analizi yapılsın."
--
-- BU SIFIRDAN BİR HESAP DEĞİL. Konu analizi 0020/0023'ten beri var
-- (`_konu_analizi`, `konu_karnesi`). Eksik olan tek şey ZAMAN KIRILIMI.
--
-- ÖLÇÜT ÜRÜNÜN GERİ KALANIYLA AYNI: "yayında + son teslim tarihi geçmiş".
-- `konu_karnesi`, `kendi_karnem` ve `veli_paneli` hep bu pencereyi
-- kullanıyor. Başka bir ölçüt seçseydik aynı ödev iki ekranda iki farklı
-- sonuç verirdi — öğretmen hangisine inanacağını bilemezdi.
--
-- ÖDEV HANGİ HAFTAYA SAYILIYOR: SON TESLİM TARİHİNE göre (öğretmenin
-- kararı). Oluşturma tarihine göre saysaydık, cuma verilip iki hafta
-- sonrasına süre tanınan bir ödev kimse teslim etmeden "o haftanın
-- analizinde" görünür ve hafta boş çıkardı.
--
-- DÖNEM = SEÇİLEN TARİH ARALIĞI (öğretmenin kararı). MEB takvimi her yıl
-- değişiyor ve ara tatiller kayıyor; "Eylül–Ocak" diye sabitlemek
-- belgeyi bir yıl sonra yanlış yapardı.
--
-- KAPSAM: öğretmen yalnız KENDİ sınıfının analizini alabiliyor
-- (`_ogretmenin_sinifi`). Sahip hepsini alabiliyor — kapsam yardımcısı
-- onu zaten kapsıyor.
-- =============================================================================
...
-- 1. EŞİKLER TEK YERDE
--
-- "İyi giden" ve "çalışılmalı" ayrımı üç yerde gerekiyor (hafta, ay,
-- dönem). Üç yerde üç kez yazılsaydı biri değişip diğerleri kalabilir ve
-- aynı konu haftalıkta "iyi", dönemlikte "çalışılmalı" görünebilirdi.
--
-- AZ VERİ AYRI BİR DURUM. İki soruluk bir konuya "çalışılması gerekiyor"
-- demek, öğretmene olmayan bir bilgi vermektir. Alt sınırın altındaki
-- konular listede DURUYOR ama 'az_veri' damgasıyla; gizlenmiyorlar,
-- yalnız iyi/zayıf listelerine girmiyorlar.
--
-- ÇİZGİ BİR KEZ YAZILIYOR. Önce sayılar iki yerdeydi: burada (0.70/0.50/5)
-- ve yanıtın `esikler` alanında ayrı bir sabit olarak. Biri değişip öteki
-- kalsaydı ekran "iyi: %70 ve üstü" yazarken sunucu %65'ten damgalar,
-- öğretmen de ekranda yazana bakıp yanlış olduğunu ANLAYAMAZDI. Artık
-- ikisi de bu yardımcıdan okunuyor ve bu dosyanın sonundaki doğrulama
-- ikisinin GERÇEKTEN aynı çizgiyi kullandığını sınıyor.
-- -----------------------------------------------------------------------------
...
-- 2. ANALİZ
--
-- Tek uçta hafta, ay ve aralık özeti birlikte dönüyor: üç ayrı uç olsaydı
-- ekran üç istek yapar ve üçü arasında zamanlama farkı oluşabilirdi.
-- Bir dönem ~20 hafta ve ~5 ay; yanıt küçük kalıyor.
-- -----------------------------------------------------------------------------
