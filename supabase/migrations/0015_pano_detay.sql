-- =============================================================================
-- SEKİZ — 0015 PANO KUTULARININ AÇILABİLMESİ
--
-- ÖĞRETMENİN İSTEĞİ:
--   "Panoda öğrenci kutusuna tıkladığımda ödev verilen öğrenci sayısı olmalı
--    ve öğrenciler sınıflara göre gruplanarak açılmalı ya da özel ders
--    öğrencisi ise bu şekilde belirtilerek açılmalı. Açık ödev ve
--    tıkladığımda ödevler görünmeli. Göndermeyene tıkladığımda göndermeyen
--    öğrenciler görünmeli, yine sınıflarına göre kategorize edilmiş.
--    Puan bekliyora tıkladığımda puan bekleyenler görünmeli."
--
-- Pano bugüne kadar dört SAYI gösteriyordu ve sayıdan ötesine gitmenin yolu
-- yoktu. "11 öğrenci göndermemiş" bilgisi tek başına eyleme dönüşmüyor;
-- öğretmenin ihtiyacı olan şey o on bir ismin kim olduğu.
--
-- TEK UÇ, DÖRT LİSTE. Dört ayrı RPC yerine tek uç: dördü de aynı gruplama
-- kuralını (sınıfa göre, özel ders ayrı işaretli) ve aynı yetki denetimini
-- paylaşıyor. Ayrı yazsaydık gruplama dört yerde ayrı ayrı bakım isterdi.
--
-- Bu dosya tekrar çalıştırılabilir.
-- =============================================================================

-- -----------------------------------------------------------------------------
-- 1. ogretmen_panosu — öğrenci kutusu artık "ödev verilen öğrenci"
--
-- Toplam öğrenci sayısı panoda bir karar aldırmıyor; öğretmen zaten kaç
-- öğrencisi olduğunu biliyor. Anlamlı olan, sistemin gerçekten kaç öğrenciye
-- ulaştığı: sınıfına en az bir ödev YAYINLANMIŞ öğrenciler.
--
-- `ogrenci_sayisi` KALDIRILMIYOR — boş sistemde "önce öğrenci ekleyin"
-- yönlendirmesi ona bakıyor. Yeni alan yanına ekleniyor.
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- 2. pano_detay — bir kutunun arkasındaki liste
--
-- Dönüş şekli dört tür için AYNI:
--   { baslik, aciklama, toplam, gruplar: [ { sinif, ozel, satirlar: [...] } ] }
--
-- Satırın alanları türe göre değişiyor ama zarf aynı; arayüz tek bir ekranla
-- dördünü de çizebiliyor. Dört ayrı şekil dönseydi dört ayrı ekran gerekirdi.
--
-- Gruplama SUNUCUDA yapılıyor: sıralama kuralı (sayısal sınıflar sırayla,
-- Özel ders en sonda) `siniflar.seviye`'den geliyor ve istemcide yeniden
