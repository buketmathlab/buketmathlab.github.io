-- =============================================================================
-- SEKİZ — 0008 ÖDEV DÜZENLEME VE YENİDEN PUANLAMA
--
-- NEDEN:
-- Ödev oluşturulduktan sonra değiştirilemiyordu. Öğretmen bildirdi:
-- "ödevin üzerine tıklayıp geriye dönüp yayınlamadan önce değişiklik
-- yapamıyorum."
--
-- EN ÖNEMLİ KISIM — YENİDEN PUANLAMA:
-- `gonderimler` tablosu dogru/yanlis/bos/puan değerlerini GÖNDERİM ANINDA
-- hesaplayıp saklıyor. Cevap anahtarı sonradan düzeltilirse bu sayılar
-- eskir. Yeniden puanlamazsak öğretmen anahtarı düzeltir, ekranda düzelmiş
-- görünür, ama öğrencinin notu yanlış kalır — sessiz ve haksız bir hata.
--
-- Bu yüzden anahtar değişince ilgili tüm gönderimler yeniden hesaplanır,
-- puanı değişen her öğrenci denetim izine yazılır (Part XLIII) ve öğretmene
-- kimin notunun ne olduğu RAPOR EDİLİR. Sessizce not değiştirmek kabul
-- edilemez.
--
-- Öğretmenin kararları: yayınlanmış ödevde her alan değiştirilebilir;
-- anahtar düzeltilince otomatik yeniden puanlama yapılır.
--
-- Bu dosya tekrar çalıştırılabilir.
-- =============================================================================

-- -----------------------------------------------------------------------------
-- 1. odev_detay — düzenleme ekranının okuduğu tam kayıt
--
-- `odevler_listesi` cevap anahtarını bilinçli olarak taşımıyor. Anahtarın
-- gerektiği tek yer burası: öğretmenin kendi ödevini düzenlediği ekran.
-- -----------------------------------------------------------------------------
...
-- 2. odev_guncelle
--
-- `tur` DEĞİŞTİRİLEMEZ: test ↔ açık uçlu geçişi mevcut gönderimlerin
-- anlamını bozar (birinde şık cevapları, diğerinde fotoğraf var). Öğretmen
-- istemedi; kapsam dışı.
--
-- `null` verilen alan "değiştirme" demek değil, "boşalt" demek olurdu;
-- bu karışıklığı önlemek için her alan açıkça gönderilir ve başlık/sınıf/
-- tarih gibi zorunlu alanlar null gelemez.
-- -----------------------------------------------------------------------------
