...
--
-- ÖLÇÜLEN EKSİK: `odev_olustur` tek sınıf alıyor (`p_sinif_id uuid`). 9A, 9B
-- ve 9C'ye aynı ödevi vermek için üç adımlı akıştan üç kez geçmek, iki PDF'i
-- üç kez yüklemek ve cevap anahtarını üç kez girmek gerekiyordu. 12 sınıf var
-- ve aynı seviyenin şubeleri aynı müfredatı görüyor.
--
-- KOPYA, ORTAK ÖDEV DEĞİL. `odevler.sinif_id` `not null` ve neredeyse her uç
-- ona göre süzüyor (0013, 0016, 0020, 0023, 0026, 0029). "Tek ödev - çok
-- sınıf" şeması o uçların tamamını elden geçirmek demekti. Bunun yerine her
-- sınıfın KENDİ ödevi oluşuyor: kendi gönderimleri, kendi karnesi, kendi
-- ortalaması. Aralarındaki tek bağ `grup_id`.
--
-- GÖVDE KOPYALANMIYOR. `odevler_coklu_olustur` ön denetimden sonra döngüde
-- mevcut `public.odev_olustur`'u ÇAĞIRIYOR. Tek oluşturma kod yolu kalıyor;
-- ikinci bir insert yazsaydık iki yol bir gün ayrışırdı.
--
-- ATOMİK. Tek işlem: hepsi ya da hiçbiri. Ön denetim geçişi atomikliği
-- SAĞLAMIYOR (onu geri alma sağlıyor); boşa iş yapmayı önlüyor. (0024'te
-- yazılan aynı not.)
--
-- `odev_detay` ve `odevler_listesi` gövdeleri KAYNAKTAN BİREBİR kopyalandı
-- (`scratchpad/uret-0030.py`); tek ekleme `kardesler`. İmzaları değişmiyor —
-- 0007 tuzağına (yeni imza, eski imzanın yetkisiyle ayakta kalması)
-- girilmiyor.
-- =============================================================================
