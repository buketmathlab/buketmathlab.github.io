-- =============================================================================
-- SEKİZ — 0026 KENDİ KONU KARNEM: ÖĞRENCİ VE VELİ DE GÖRSÜN
--
-- ÖĞRETMENİN İSTEĞİ: "öğrenci ve veli kendi konu karnesini görsün."
--
-- 0023'te konu karnesini yazarken bunu BİLEREK dışarıda bırakmış ve sebebini
-- yazmıştım: dönem geneli "zayıf konular" listesini bir çocuğa göstermek ayrı
-- bir karardı (hangi tonda, kimin ağzından). O karar bu turda verildi.
--
-- BUGÜN NE VAR — ölçüldü:
--   konu_karnesi (0023)          → ÖĞRETMENE ÖZEL; öğrenci/veli 42501 alıyor
--   ogrenci_odevleri.konu_analizi → yalnız TEK BİR GÖNDERİM için
--   veli_paneli … konu_analizi    → yalnız TEK BİR ÖDEV için
--
-- Yani "dönem boyunca hangi konuda zayıfım" sorusu ikisinde de cevapsız.
--
-- -----------------------------------------------------------------------------
-- PARAMETRE ALMIYOR — VE BU TASARIMIN ÇEKİRDEĞİ
--
-- `p_ogrenci_id` alsaydı "başkasının karnesini isteyemez" bir DENETİM olurdu:
-- yazılır, unutulur, bir gün bir düzenlemede düşer. Parametre hiç olmayınca
-- başkasının karnesini istemek YAPI GEREĞİ imkânsız — sorulacak bir kimlik
-- yok, öğrenci oturumdan geliyor. (`ogrenci_mesajlari` ve `veli_paneli` ile
-- aynı desen.)
--
-- -----------------------------------------------------------------------------
-- `konu_karnesi`'YE DOKUNULMUYOR
--
-- İmzası ve `_ogretmen` şartı 0023'teki gibi kalıyor; 0007 tuzağına hiç
-- girilmiyor ve `konu_karnesi_testleri.sql` 10. grubunun güvencesi
-- ("öğrenci ve veli çağıramıyor") bozulmuyor.
--
-- ÖLÇÜTLER O GÖVDEDEN BİREBİR KOPYALANDI, ezberden yazılmadı (0016 dersi):
-- aynı `d.yayinda and d.son_tarih < bugun_tr`, konular yalnız `d.tur='test'`,
-- gelişimde açık uçlu da var, puan `coalesce(ogretmen_puan, puan)`. Test iki
-- ucun aynı sayıyı verdiğini ayrıca ölçüyor; ayrışırlarsa kırılıyor.
--
-- -----------------------------------------------------------------------------
-- NE DÖNMÜYOR VE NEDEN
--
--   sınıf mevcudu / ortalaması → bir çocuğa "sınıfın neresindesin" demek bu
--                                ekranın işi değil; kıyas çağrıştıran hiçbir
--                                sayı gönderilmiyor
--   başka öğrencinin verisi    → sorgu `k.id = <kendi id>` ile bağlı
--   cevap anahtarı             → Kural 6; `_konu_analizi` yalnız SAYI döndürür
